...
ただし、すべてのゲートが同じ強さではありません。テストをクリーンな `HEAD` で走らせ直す、スキャナを実際に動かす、`git archive` の中身を実際に見る——こうした「自分で測り直す」ゲートは、すり抜けようとするエージェントにも通用します。一方、mumei が自分の進捗について書き出すファイルは、壁ではなく記録です。どちらがどちらかは [何が成立し、何が成立しないか](#何が成立し何が成立しないか) に書いてあります。
