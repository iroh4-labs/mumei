...
flowchart LR
  ...
  V -->|spec| S["requirements / design / tasks<br/>各々 auto-iter ≤ 3 回<br/>3 spec reviewer"]
  ...
  V -->|plan| PM["plan mode<br/>(Shift+Tab × 2)<br/>ExitPlanMode で capture"]
  PM --> TL["TaskCreate / TaskUpdate<br/>L-T1 / L-T2 counter<br/>全完了で pending_review"]
...
| `/mumei:plan [feature]`       | 新規 feature では vehicle picker (`spec` = フル SDD / `plan` = Claude plan-mode ラッパー) を出します。既存 feature は自動 resume。spec vehicle: clarification → requirements → design → tasks (各々最大 3 回 auto-review) → 単一承認 → Wave by Wave → review。                     |
| `/mumei:review`               | plan vehicle 専用の review pipeline です。最後の `TaskCompleted` で `task_created_count == task_completed_count` になり `pending_review=true` がセットされたら、Stage 0 detector + security-reviewer + adversarial-reviewer + per-issue validator を現在の diff に対して回します。 |
| `/mumei:archive <feature>`    | `done` になった feature を `.mumei/archive/<YYYY-MM>/<feature>/` に移動します。vehicle (specs/ または plans/) を自動判定し、`scratch/<feature>.md` も `scratch.md` として一緒に持ち越します。                                                                                      |

## ふたつの vehicle: `spec` と `plan`

mumei は **Quality Enforcement Layer** であり、本質的な仕事は Hook による phase 遷移 / commit-push gate / review 完了の物理強制です。その gate に向かって feature を **駆動する流儀** が **vehicle** で、mumei は 2 種類提供します:

- **`spec`** — 上で説明したフル SDD ワークフロー。`requirements.md` / `design.md` / `tasks.md` を起草し、3 つの独立した spec reviewer を回し、ユーザーに 1 回だけ承認 gate を提示してから Wave 単位で commit を切りつつ実装し、最後に 4-stage review を回します。User Story や EARS の AC、アーキテクチャ図を明示したくなる規模の feature 向きです。
- **`plan`** — Claude Code 公式の plan mode と `TaskCreate` を薄く包むだけ。`/mumei:plan` で plan vehicle を選んだら `Shift+Tab` を 2 回押して plan mode に入り、計画を accept、Claude に task を実行させるだけです。mumei は plan を `.mumei/plans/<slug>/plan.md` に capture、`TaskCreated` / `TaskCompleted` hook で進捗を tracking し、全 task 完了 (`pending_review=true`) で session 離脱と `git push` を block して `/mumei:review` の実行を促します。

両 vehicle とも、review pipeline (Stage 0 detector + security + adversarial + per-issue validator)、`MUMEI_BYPASS=1` の escape hatch、`/mumei:archive` での後始末は同じです。違うのは pipeline への **入り方** だけ:

| 観点              | `spec`                                               | `plan`                                                       |
| ----------------- | ---------------------------------------------------- | ------------------------------------------------------------ |
| 起草 artifact     | requirements.md / design.md / tasks.md               | plan.md (`ExitPlanMode` から capture)                        |
| ユーザー承認      | 1 回 (3 spec reviewer 全 PASS の後)                  | 0 回 (plan mode への hand-off が contract)                   |
| 実装              | Wave-by-Wave、`_Files:_` scope + Wave ごとに commit  | plan mode で task 一覧を自由実行                             |
| Session 離脱 gate | Stop hook (R1) — 全 task `[x]` + 通過 review         | Stop hook (L-R1) — `pending_review=true` + 通過 review       |
| `git push` gate   | PreBash (R2) — review verdict ≠ MAJOR_ISSUES         | PreBash (L-R2) — 同左、参照は `.mumei/plans/<slug>/reviews/` |
| Review 起点       | `/mumei:plan` Phase 5                                | `/mumei:review`                                              |
| 向いている用途    | 新規 feature で scope が大きいもの                   | bug fix、軽量 feature、別 SDD tool 併用プロジェクト          |
| Reviewer 起動     | spec-compliance + security + adversarial + validator | security + adversarial + validator (spec-compliance なし)    |

SDD ワークフロー自体が作業より重く感じるなら `plan` を、要件とコードのトレーサビリティに摩擦の価値があると思えば `spec` を選んで下さい。
...
| 変数                     | デフォルト | 効果                                                                                                                                                                                                 |
| ------------------------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MUMEI_DETECTOR_TIMEOUT` | `600`      | detector ごとの wall-clock timeout (秒)。`semgrep` / `osv-scanner` 共通です。巨大なリポジトリでは引き上げてください。CI などで「ハングした detector のほうが見逃しより困る」状況なら下げてください。 |
...
mumei は **完全にローカルで動作します**。mumei 自身からの outbound 通信はありません。mumei が呼び出す third-party の `osv-scanner` が CVE データ取得のために `osv.dev` に問い合わせますが、これは tool 側の挙動です ([PRIVACY.md](./PRIVACY.md) を参照)。

| 項目             | 内容                                                                                                                                         |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| **外部通信**     | mumei 自身からはありません。`osv-scanner` (third-party) が CVE データ取得のため `osv.dev` に問い合わせます。mumei はその挙動を制御しません。 |
