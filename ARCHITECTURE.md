...
│   ├── pre-edit-guard.sh   # P1 / P2 / P3 / I1 / I2 / W1 / M1 / S1 / S3 / G1 / E1
│   ├── pre-bash-guard.sh   # I3 / I5 / R2 / W2 / M2 / S2 / S4 / G2 / G3
...
│   ├── session-start-bypass-notice.sh  # X6 (SessionStart): announce an active MUMEI_BYPASS — the one hook the bypass does not silence
...
│   ├── config-change-audit.sh  # ConfigChange: audit + invalid JSON exit 2 + X7 (MUMEI_BYPASS turned on via settings)
...
| R2   | review       | PreToolUse(Bash)         | `git push` while latest review verdict is `MAJOR_ISSUES`, OR a clearing verdict (`PASS` / `NEEDS_IMPROVEMENT`) is not backed by reviewer execution anchored to the pushed diff — a hollow or stale review (a baseline reviewer's cost-log `diff_hash` does not match the gating review's, the current repo state no longer hashes to the gating `diff_hash` = a re-edit since the verdict, the gating review carries no `diff_hash`, OR the resolved gating review is itself `MAJOR_ISSUES`); see `mumei_review_trace_ok` in `hooks/_lib/review.sh`. A clearing push that passes the trace also emits a **non-blocking advisory** when memory curation was skipped (reviewer `memory_candidates` emitted but no `memory-curator` ran for the gating diff — `mumei_review_curator_complete`) | `hooks/pre-bash-guard.sh`              |
| R3   | done         | Stop                     | `phase=done` but feature still in `.mumei/current` (nags at most once per `session_id`, recorded in `state.json` `shelve_nag_session`, so in-flight PR landing is not re-blocked every stop)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `hooks/stop-guard.sh`                  |
...
| M2   | any          | PreToolUse(Bash)         | Bash-route write to `.claude/agent-memory/<reviewer>/MEMORY.md` (the curator's `mumei_memory_apply_operation` writes via an internal mktemp + mv, so it is unaffected)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `hooks/pre-bash-guard.sh`              |
| S1   | any          | PreToolUse(Edit)         | LLM-driven Edit/Write on mumei harness state: `.mumei/current` / `.mumei/config.json` / state.json / cost-log.jsonl / spec-reviews/_.json / reviews/_.json (orchestrator helpers only)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `hooks/pre-edit-guard.sh`              |
| S2   | any          | PreToolUse(Bash)         | Bash-route write to mumei harness state: state.json / cost-log.jsonl / spec-reviews/_.json / reviews/_.json, plus `.mumei/config.json` once it exists (so `golden_paths` cannot be edited to unlock G1). `.mumei/current` is excluded — the orchestrator's own skills write the pointer with a literal redirect                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `hooks/pre-bash-guard.sh`              |
| S3   | any          | PreToolUse(Edit)         | Edit/Write on `.claude/settings*.json` whose content introduces `MUMEI_BYPASS` — settings `env` reaches hook processes, so one line disables every gate, and `settings.local.json` is gitignored so the change appears in no diff. Content-scoped: ordinary settings edits are untouched                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `hooks/pre-edit-guard.sh`              |
| S4   | any          | PreToolUse(Bash)         | Bash-route write that puts `MUMEI_BYPASS` into `.claude/settings*.json` (the Bash half of S3)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `hooks/pre-bash-guard.sh`              |
...
| X6   | any          | SessionStart             | `MUMEI_BYPASS=1` is active — announced to the user and to Claude (advisory). The ONLY hook that does not short-circuit on the bypass: a harness that is off must say it is off                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `hooks/session-start-bypass-notice.sh` |
| X7   | any          | ConfigChange             | A settings write turned `MUMEI_BYPASS` on (advisory + audit-log entry). Fires on the resulting state, not on the command, so it catches the writes S3 cannot read                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `hooks/config-change-audit.sh`         |
