...
| X4   | implement    | PreToolUse(Bash)         | Record the I3 commit-gate test result (exit code) to `verify-log.jsonl` (internal, no deny); `MUMEI_TEST_CMD` overrides runner auto-detect                                                 | `hooks/pre-bash-guard.sh`     |
