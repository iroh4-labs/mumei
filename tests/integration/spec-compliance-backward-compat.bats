...
#   4. skills/plan/SKILL.md Phase 5 Stage 1 wires the spec-vehicle
#      scope_source (REQ-17.5)
#   5. skills/review/SKILL.md Step 6 wires the plan-vehicle scope_source
...
@test "skills/plan SKILL.md Phase 5 Stage 1 wires spec-vehicle scope_source" {
  ...
  grep -A2 'scope_source' "$CLAUDE_PLUGIN_ROOT/skills/plan/SKILL.md" |
    grep -q 'requirements\.md'
}

# ─── Plan-vehicle wiring (REQ-17.6) ──────────────────────────

@test "skills/review SKILL.md Step 6 wires plan-vehicle scope_source" {
  grep -qE 'scope_source=\.mumei/plans/.+/plan\.md' \
    "$CLAUDE_PLUGIN_ROOT/skills/review/SKILL.md"
}

@test "skills/review SKILL.md Step 6 launches spec-compliance-reviewer" {
  # Phrase: 'Task(subagent_type: "spec-compliance-reviewer", ...)'
  grep -qE 'subagent_type:[[:space:]]*"spec-compliance-reviewer"' \
    "$CLAUDE_PLUGIN_ROOT/skills/review/SKILL.md"
}

@test "skills/review SKILL.md Step 8.5 includes spec-compliance in curator loop" {
