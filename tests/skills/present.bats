...
@test "present: .mumei/current points to non-existent feature → 'no active feature' + exit 0" {
  mkdir -p .mumei
...
@test "present: active feature with empty log → N/A one-liner (REQ-25.2.1)" {
...
@test "present: explicit feature arg overrides .mumei/current (REQ-25.2.2)" {
  mkdir -p ".mumei/specs/REQ-1-active"
  mkdir -p ".mumei/specs/REQ-2-other"
  printf '%s\n' "REQ-1-active" >.mumei/current
  run _run_present "REQ-2-other"
  [[ "$status" -eq 0 ]]
  [[ "$output" == "REQ-2-other | pass^3: N/A (n=0, window=10, k=3)" ]]
}

@test "present: populated log → numeric value in one line" {
...
@test "present: output is exactly one line (no trailing blank, no header)" {
