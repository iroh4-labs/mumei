...
| **escape hatch** | `MUMEI_BYPASS=1` 環境変数。単一のルールで、監査可能。ルール単位の迂回やフィーチャーフラグはありません。エージェントには立てさせません: `.claude/settings*.json` への書き込みは拒否され、有効なまま始まったセッションは開始時にそれを申告します。 |
...
下の表は、hook 層で使われる `MUMEI_*` 名をまとめたものです。
多くのユーザーが必要とするのは、1 コマンド用の escape hatch である
`MUMEI_BYPASS` と、標準ではない test runner を使うときの
`MUMEI_TEST_CMD` です。内部用と書かれた項目は grep 結果には現れますが、
ユーザー向けの公開設定項目ではありません。

| Variable | Purpose | Default / notes |
| --- | --- | --- |
| `MUMEI_BYPASS` | フックのゲートを強制せず終了する、1 コマンド用の escape hatch。立てるのは**あなた**で、エージェントではありません: `.claude/settings.json` / `settings.local.json`（プロジェクト・ユーザー共通のどちらも）へ書き込む Edit / Bash は拒否され、設定変更で有効化されれば警告と監査ログが残り、有効な状態で始まったセッションは「全ゲートが無効」であることを申告します。設定の `env` はフックのプロセスに届き、`settings.local.json` は gitignore されているため、これがなければ **diff に一切現れないファイルでエージェントが切れるスイッチ**になってしまいます。 | 未設定。単一コマンドだけ `1` にする。 |
| `MUMEI_TEST_CMD` | コミットゲートのテストコマンド自動検出を上書きする。 | 未設定。一般的な project manifest から自動検出する。 |
| `MUMEI_DEBUG` | 共通ログヘルパーの debug ログを有効にする。 | `0`。`1` で debug 行を出力する。 |
| `MUMEI_CONTEXT_LINES` | subagent に注入する artifact context の行数上限。 | `200`。 |
| `MUMEI_COMPACT_HINT_PCT` | proactive `/compact` ヒントを出す context 使用率しきい値。 | `60`。 |
| `MUMEI_CONTEXT_MAX_TOKENS` | proactive `/compact` ヒント計算で使う model context window サイズ。 | `1000000`。 |
| `MUMEI_DETECTOR_FAILED` | pre-review detection 中に binary が crash した detector の内部 JSON 配列。 | `pre-review-detector.sh` が設定する。ユーザー指定値ではない。 |
| `MUMEI_DETECTOR_TIMEOUT` | detector ごとのタイムアウト秒数。 | `600`。 |
| `MUMEI_DETECTOR_SEMGREP_MIN` | `semgrep` の推奨最小バージョン。 | `1.100.0`。 |
| `MUMEI_DETECTOR_OSV_SCANNER_MIN` | `osv-scanner` の推奨最小バージョン。 | `2.0.0`。 |
| `MUMEI_DETECTOR_TIER2` | 任意の Tier2 detector を有効にする。 | `0`。`1` で Tier2 detector を含める。 |
| `MUMEI_CODEQL_DB` | 任意の CodeQL detector が使う事前ビルド済み CodeQL database のパス。 | 未設定。directory を指していなければ CodeQL は skip される。 |
| `MUMEI_BYTE_EXACT_EXTS` | byte-exact helper が確認する拡張子のスペース区切り一覧。 | `.go .bat .cmd`。 |
| `MUMEI_LOG_MAX_MB` | append-only log を切り詰めるサイズしきい値。 | `10`。 |
| `MUMEI_LOG_MAX_LINES` | hook コメントで参照される log rotation の保持行数。 | `5000`。helper 内では固定値として扱われる。 |
| `MUMEI_LEDGER_MAX_LINES` | cross-feature finding ledger に残す最大行数。 | `5000`。 |
| `MUMEI_LEDGER_PATH` | cross-feature finding-ledger のパスを上書きする。 | project-local `.mumei/finding-ledger.jsonl`。主にテスト用。 |
| `MUMEI_DETECTOR_REGISTRY` | detector registry を上書きする。 | `semgrep osv-scanner`。高度な hook 開発向け。 |
| `MUMEI_MEMORY_THRESHOLD` | memory-curator の内部スコアしきい値。 | `15`。公開設定項目ではない。 |
| `MUMEI_MEMORY_MAX_ENTRIES` | memory-curator の内部 entry 上限。 | `30`。公開設定項目ではない。 |
| `MUMEI_MEMORY_MAX_BYTES` | memory-curator の内部入力サイズ上限。 | `8192`。公開設定項目ではない。 |
| `MUMEI_MEMORY_FINAL_TEXT_MAX_BYTES` | memory-curator の内部 final-text 上限。 | `1024`。公開設定項目ではない。 |
| `MUMEI_MEMORY_AXES` | memory-curator の内部 scoring-axis 配列。 | helper 内の固定値。公開設定項目ではない。 |
| `MUMEI_WT_RAN` | worktree verification helper が設定する内部戻り値フラグ。 | mumei が設定する。ユーザー指定値ではない。 |
| `MUMEI_WT_TAIL` | worktree verification helper が設定する内部戻り値テキスト。 | mumei が設定する。ユーザー指定値ではない。 |
