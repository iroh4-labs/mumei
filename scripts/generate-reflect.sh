#!/usr/bin/env bash
# Generate reflect.md for a finished mumei feature. Called by the
...
#
# Usage: bash scripts/generate-reflect.sh <feature_dir>
#
...
if [[ -z "$feature_dir" || ! -d "$feature_dir" ]]; then
  echo "generate-reflect: invalid feature_dir: ${feature_dir}" >&2
  exit 1
fi

retro_path="${feature_dir}/reflect.md"
if [[ -e "$retro_path" ]]; then
  retro_path="${feature_dir}/reflect-$(date -u +%Y%m%dT%H%M%SZ).md"
fi
...
# Compose the markdown.
{
  ...
  # shellcheck disable=SC2016  # backticks here are literal Markdown code spans
  printf -- '_Generated by `scripts/generate-reflect.sh` on %s._\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
} >"$retro_path"
