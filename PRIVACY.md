...
mumei itself does not push these files anywhere. `/mumei:kindle` writes `.mumei/.gitignore` that ignores **only** per-developer state (`current`, `specs/*/state.json`); everything else under `.mumei/` — `scratch/`, `specs/*/{requirements,design,tasks}.md`, `spec-reviews/`, `reviews/`, `cost-log.jsonl`, `archive/` — is intentionally **tracked** for team handoff. If you do not want these committed, edit `.mumei/.gitignore` after `/mumei:kindle` or add `.mumei/` to the project-root `.gitignore`.

## Contact

Open an issue: https://github.com/iroha924/mumei/issues
